...
public final class VInt {
    ...
    public static Result read(final byte[] data, final int offset) {
        ...
        while (bytesRead < MAX_BYTES) {
            final int pos = offset + bytesRead;
            if (pos >= data.length) {
            ...
            if ((b & CONTINUATION_FLAG) == 0) {
                return new Result(value, bytesRead);
            }
    ...
    public static int write(final long value, final byte[] data, final int offset) {
        ...
        do {
            final int pos = offset + bytesWritten;
            if (pos >= data.length) {
                throw new IllegalArgumentException("insufficient space in data array");
            }

            final int b = (int) (remaining & DATA_MASK);
            remaining >>>= 7;

            if (remaining != 0) {
                data[pos] = (byte) (b | CONTINUATION_FLAG);
            } else {
                data[pos] = (byte) b;
            }
