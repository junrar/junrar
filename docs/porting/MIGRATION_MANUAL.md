...
Issue citations: bare `#NN` numbers are upstream `junrar/junrar` issues (historical
bugfix provenance). Port-tracker work is cited as `PT-NN`, defined exclusively in
[Appendix A](#appendix-a-port-tracker-issue-index) with a self-contained summary per
key, so those citations survive if the tracker they came from goes away.
...
- Java arrays cap at 2^31 bytes — dictionaries > 2 GB need a segmented-window
  ...
  M4.3** (PT-35, Appendix A): `Unpack5Window` is a `byte[][]` of 256 MB power-of-two
...
| T3 | **RarVM precedence bugs** — DELETED on rar5-port by chunk M2.2 (PT-21, Appendix A); description kept as a never-copy exemplar | (was) `RarVM.java:250-251,300-393,509,565-609,636` | (a) `a & 0xFFffFFff + b` parses as `a & (0xFFffFFff + b)` — VM_ADDB/SUBB/INC/DEC/MUL/ADC/SBB all wrong; (b) ternary-vs-`\|` flag computation drops the sign bit + signed compare where C++ is unsigned; (c) VM_SAR uses `>>>` where C++ is arithmetic `>>`; (d) `prepare()` copies code with `inBuf[i] \|= code[i]` — a second prepare() can OR stale bytes. Survived because real archives hit the standard-filter path. **The bugs left with the code**: M2.1 (PT-20, Appendix A) replaced the interpreter with 5.5.1 fingerprint recognition, M2.2 then deleted the dead executor + opcode enums (mirrors unrar `1522ee0` = 5.5.1). **Rule (still applies to any future VM code): parenthesize every masked term; per-opcode unit tests for anything new.** |
...
## Appendix A: port-tracker issue index

The RAR5/RAR7 port was developed against a working tracker on a fork
(`andrebrait/junrar`) that may not exist forever. `docs/porting/` therefore never
cites those issues by bare `#NN`; every citation uses a **`PT-NN` key**, and this
table is the sole definition of each key — self-contained, so the citations stay
meaningful if the tracker disappears. Bare `#NN` numbers elsewhere in these
documents (the §6 trap rows and the no-go S/C rows: #8, #14, #31 in S7, #36/#45 in
C9, #86, #88, #90, #104, #108, #110, #216) are upstream `junrar/junrar` issues —
this repository's own tracker — and keep their bare form. Note the collisions this
avoids: upstream #31 (S7's 2019 `directoryTraversalBug` fix, `e60b915f`) vs PT-31,
and upstream #36/#45 (C9's 2020 corrupt-header NPE reports) vs the port tracker's
own 36/45.

| Key | Origin | Chunk | What it did |
| --- | --- | --- | --- |
| PT-12 | originally `andrebrait/junrar#12` | P0.7 | RAR3 header-CRC verification scaffolding: 16-bit `GetCRC15` coverage with unrar's legacy exemptions, and the record-vs-fatal tolerance contract (broken headers are marked and refused at extract time, not at open). |
| PT-18 | originally `andrebrait/junrar#18` | M1.4 | `FirstWinDone` distance validation (unrar 7.0.3 parity): a match distance reaching into the never-written part of the window zero-fills instead of self-copying stale bytes. |
| PT-20 | originally `andrebrait/junrar#20` | M2.1 | Replaced the RarVM bytecode interpreter with unrar-5.5.1-style standard-filter fingerprint recognition ({length, CRC32} against the six canonical filters). |
| PT-21 | originally `andrebrait/junrar#21` | M2.2 | Deleted the dead VM interpreter, opcode enums and parse-only VM types (§6 T3 surface removal); mirrors unrar `1522ee0`. |
| PT-27 | originally `andrebrait/junrar#27` | M3.6 | `Unpack5` sibling-engine skeleton: per-archive window, bit input, block-header read, the five RAR5 Huffman tables, and the three-part dictionary resource model. |
| PT-31 | originally `andrebrait/junrar#31` | M3.10 | `FHEXTRA_REDIR` link extraction with the three symlink-safety layers; unsafe targets throw `UnsafeLinkException`. (Distinct from upstream #31 in S7.) |
| PT-34 | originally `andrebrait/junrar#34` | M4.2 | RAR7 extended-distance decode (`DCX`=80 alphabet, `DBits>36` arm), distance state widened to `long` end to end, version-70 routing via `FileHeader.isRar5Family()`. |
| PT-35 | originally `andrebrait/junrar#35` | M4.3 | Segmented window above 1 GB (`byte[][]` of lazily allocated 256 MB segments), engine capability raised to 64 GB, positions wrap instead of masking (RAR7 dictionaries are not powers of two). |
| PT-40 | originally `andrebrait/junrar#40` | M3.10 follow-up | Investigation that closed the three link-path divergences as working-as-intended: keep junrar's fail-closed reject over unrar's sanitize-and-continue (no-go row D3). |
| PT-44 | originally `andrebrait/junrar#44` | post-M4 | RAR3 narrow-name decode moved to the 7.2.7 rule: UTF-8 validity scan with a lossless ISO-8859-1 fallback (`FileHeader.decodeNarrowName`, §6 T6 round 3, no-go row D4); exactly one corpus member flipped. |
