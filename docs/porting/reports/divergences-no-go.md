...
Issue citations: bare `#NN` numbers are upstream `junrar/junrar` issues. Port-tracker
work is cited as `PT-NN`, defined exclusively — with a self-contained summary per key —
in `../MIGRATION_MANUAL.md` Appendix A ("port-tracker issue index").
...
| D3 | **FHEXTRA_REDIR link-path reject-vs-strip, three cases (PT-40, follow-up to PT-31/M3.10)**: (1) a `UNIX_SYMLINK` target containing a backslash (`..\zz\bad`), (2) writing a member through a previously-extracted directory-symlink (`LinksToDirs`), and (3) a `HARDLINK`/`FILE_COPY` target containing `../` traversal — junrar rejects all three with `UnsafeLinkException`; unrar 7.23 sanitizes-and-continues (creates the symlink literally, deletes+recreates the dir, strips the traversal via `ConvertPath`). **Investigated 2026-07-21 and CLOSED as working-as-intended — KEEP the reject on all three, do not adopt unrar's strip-and-continue.** Ecosystem cross-check (executed against local jars): Apache Commons Compress `1.28.0`/`1.26.1` `ZipArchiveEntry.setName` (`archivers/zip/ZipArchiveEntry.java:1404-1409`) replaces `\`→`/` for default (`PLATFORM_FAT`) entries at header-parse time, called from both `ZipArchiveInputStream.readNextEntry` and `ZipFile`'s central-directory parse — i.e. mature libraries treat backslash as a separator to sanitize, not a literal filename byte, matching junrar's S5 posture (case 1). zip4j (`master`, fetched from `github.com/srikanth-lingala/zip4j`) `AbstractExtractFileTask.getFileNameWithSystemFileSeparators` (`tasks/AbstractExtractFileTask.java:191-193`) treats **both** `/` and `\` as directory separators when building the output path, and its own zip-slip guard `assertCanonicalPathsAreSame` (`:74-92`) **throws** `ZipException` on an escaping canonical path rather than stripping and continuing — the same canonical-containment-then-reject junrar already applies (S6/S7). No Java zip/7z library has a native dir-symlink-write-through or hardlink-redirection analogue to cross-check cases 2/3 directly, but the reject-over-strip precedent from case 1 and zip4j's own zip-slip reject extend by the same reasoning: unrar's `LinksToDirs` deletes whatever filesystem object occupies that path (attacker-planted or pre-existing, without checking its target) before recreating a directory, which is a strictly riskier operation than junrar's fail-closed refusal; and even though unrar's `ConvertPath` strip for case 3 is provably safe (`out/secret.t` stayed inside the destination in the executed local probe, see PT-40), adopting "sanitize instead of reject" on any traversal-adjacent path is inconsistent with junrar's own CVE-2026-28208/CVE-2026-41245 fail-closed philosophy for no compatibility benefit that outweighs that consistency. **No code changed** — this is a documentation-only decision; per PT-40 acceptance, moving any of the three to parity would itself need to update this row and requires test-first RED→GREEN plus independent adversarial verification, neither of which is warranted absent a concrete correctness/compat complaint. | M3.10 (`410a6468`), PT-31 / PT-40 | `LocalFolderExtractor.java` `validateSymlinkTarget` (case 1), `refuseWriteThroughSymlink` (case 2), `resolveTargetWithinDestination` (case 3) | `ArchiveRar5LinkTest` hostile rows (`h-bsl`, `dqqq/evil.txt`, `h-hard.txt`); `src/test/resources/com/github/junrar/links/README.md` |
| D4 | **RAR3 narrow-name decode: UTF-8 on valid input, byte-transparent ISO-8859-1 on invalid (PT-44, manual §6 T6 round 3)**: unrar 7.2.7 sends every narrow RAR3 name — the plain-ANSI field and the LHD_UNICODE-without-NUL-split field alike — through `ArcCharToWide(..., ACTW_OEM)` (`d861246:arcread.cpp:359-360`), a strict multibyte decode in the process locale, i.e. UTF-8 on any modern Unix. junrar ports the success branch (valid UTF-8 → decoded as UTF-8; Java's decoder is stricter than macOS `UtfToWide`, which checks only continuation bits and the 0x10FFFF ceiling — Java also rejects overlong forms, surrogate code points and 5/6-byte sequences, matching glibc `mbsrtowcs`, and every extra rejection falls through to the lossless fallback) but **deliberately does NOT port the failure branch**, because unrar's is lossy and platform-split: macOS `_APPLE` builds call `UtfToWide` unconditionally and truncate the name at the first invalid byte (`unicode.cpp:95-96`, `416-488`; `ArcCharToWide` discards the `false` return — executed: `unrar 7.23 lb rar3-ansi-name.rar` prints `caf` for the name field `caf\xe9.txt`), while glibc builds remap every byte `>= 0x80` into the U+E000 private-use area behind a U+FFFE marker (`CharToWideMap`, `unicode.cpp:203-248`). junrar keeps the byte-transparent ISO-8859-1 decode instead: it is lossless, round-trips through `getFileNameByteArray()`, and is the same fail-safe choice for the CP1251/CP936/Windows-1252 legacy-codepage names that dominate the RAR3 era. Decoding invalid input as UTF-8 with replacement is expressly excluded — it destroys those bytes as U+FFFD. | PT-44 (manual §6 T6 round 3), superseding the 3.7.3-pinned round 2 | `FileHeader.java` `decodeNarrowName`, called at three sites: the plain-ANSI field, the LHD_UNICODE-without-NUL-split field (both are literally unrar's `hd->FileName.empty()` fallback), and — junrar-only — the ANSI half of a NUL-split LHD_UNICODE field. unrar has no counterpart for that third site because it keeps one name, not two: when `EncodeFileName::Decode` succeeds, `hd->FileName` is non-empty and the narrow half is discarded. junrar retains it as `fileName` and falls back to it whenever `fileNameW` decodes empty (no-go C4) — which is precisely the `hd->FileName.empty()` case unrar does convert — so decoding it the same way is what keeps that fallback faithful. Two knowing deltas against the C++, both probed rather than assumed: (1) unrar passes a C string and stops at the first NUL where junrar scans the whole `nameSize` field — unobservable, because an embedded NUL survives both decodes as U+0000 and `isFilenameValid` rejects every such name (executed: `new File("valid\0junk").getCanonicalPath()` throws `IOException: Invalid file path`), so the header is refused either way; (2) the strictness delta described above. The all-low-ASCII fast path of `strfn.cpp:42-57` is ported too, which also avoids allocating a `CharsetDecoder` per name | `FileHeaderNarrowNameUtf8Test` → `unicode/rar3-ansi-utf8-name.rar` (valid UTF-8, decoded); `FileHeaderAnsiNameTest` → `unicode/rar3-ansi-name.rar` (invalid UTF-8, byte-transparent); `FileHeaderUtf8NameTest.splitNameAnsiHalfGetsTheSameNarrowDecode` → `unicode/rar3-utf8-name-split.rar` (the third site, asserted through the deprecated `getFileNameString()` since `getFileName()` returns the RLE half — which is also why the regression corpus cannot see that site: `FileHeaderRecord` records `getFileName()` only); corpus member `commoncrawl3/3I/3ILZMJBO2TG2LCBV6L5AHMXEYLEXH4X2` (the single flip of 12,475) |
...
*before* `Archive.extractFile`, which is where the P0.7/PT-12 broken-header refusal throws. By then
